...
def conv2d(layer, weights, stride):
...
def new_conv_layer(layer, num_input_channels, filter_size, num_filters, stride, use_pooling=True):
    ...
    layer = conv2d(layer, weights, stride) + biases
    
    layer = tf.nn.relu(layer)
    
    if use_pooling:
        layer = lrnmaxpool(layer)
...
##model: x, keepprob, 64, 3, 3, 128, 2
    ...
    with tf.name_scope('Fully_Connected1'):
        ...
        flat_layer, num_features = flatten_layer(layer_conv5)
...
#        dropout_layer = dropout(layer_fc1, keep_prob)
    ...
    with tf.name_scope('Fully_Connected3'):
        ...
        layer_fc3 = new_fully_connected_layer(
            layer_fc2,
            num_inputs=256,
            num_outputs=2,
            use_relu=False,
...
def train(img_dir, model_dir, img_size=224, colour_channels=3, batch_size=128, training_epochs=50):
